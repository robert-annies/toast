...
class ConfigTest < ActiveSupport::TestCase
  ...
  context 'expose directives' do
    should 'register models [A.1]' do
      ...
      assert_equal 'Apple', Toast.expositions.first.model.name
      assert_equal 'Banana', Toast.expositions.second.model.name
    end
    ...
    should 'register associations [A.9]' do
      ...
      assert_equal 'Apple',                  a.associations[:bananas].base_model.name

      assert_equal 1,                         b.associations.size
      assert_equal :apple,                    b.associations[:apple].assoc_name
      assert_equal 'Banana',                  b.associations[:apple].base_model.name
    end

    should 'register via_get, via_put and via_delete directives [A9,A10,A11]' do
      Toast.init 'test/files/via_verb.rb'
      config = Toast.expositions.first

      assert_equal OpenStruct, config.via_get.class
      assert_equal OpenStruct, config.via_put.class
      assert_equal OpenStruct, config.via_delete.class
      assert_equal nil,        config.via_post
      assert_equal nil,        config.via_link
      assert_equal nil,        config.via_unlink
    end
  ...
  context 'association directives' do
    should 'register via_get, via_post, via_link and via_unlink configs [B.1]'  do
      ...
      assert_equal nil, config.associations[:apple].via_post
    ...
    should 'raise error when association missing' do
      ...
      assert_equal "CONFIG ERROR: Model association expected\n"+
    ...
    should 'register max window size [B.2]' do
      ...
      assert_equal nil, b.associations[:apple].max_window  # singular
  ...
  context 'via_* directives' do
    ...
    should 'raise error on invalid "allow" rules [D.1]' do
      ...
      assert_equal "CONFIG ERROR: Allow rule must take exactly 3 arguments or a variable (*) argument list\n"+
  ...
  context 'collection directives' do
    should 'register via_get and via_post configs [C.1]'  do
      ...
      assert_equal nil, config.collections[:query].via_post
    end
